...
результати футбольних матчів, засетлені оптимістичним мульти-джерельним оракулом, з
AI-пундитом-суперником і free-to-play воронкою.**

> **Статус оракула:** `CupOracleV2` — оптимістичний; результат пропонує оператор,
> захищений вікном оскарження + мульти-джерельним кворумом. Економічне зміцнення
> (bonds + slash) описане в `docs/xcup/HARDENING-PLAN.md`; правила резолюції —
> в `docs/xcup/SETTLEMENT-RULES.md`.

- **Герой** — real-money USDC parimutuel-ринок (не F2P-гра, не CLOB, не AMM).
- **Ядро** — `CupOracleV2`: multi-source · quorum · оптимістичне вікно оскарження. Ринок
  тільки читає фіналізований результат → виплати un-riggable.
...
- **Чому блокчейн, а не звичайний бек:** без перевірюваної резолюції лідерборд і виплати
  = «повірте нам». On-chain parimutuel-контракт робить **гроші** непідкручуваними
  (виплати — чиста математика, оператор їх не торкається); **результат** —
  operator-proposed під вікном оскарження й опублікованими правилами, а bonded-оракул
  із HARDENING-PLAN робить і результат економічно непідкручуваним.
...
### 7.2 Тест-план (Hardhat, локальний X Layer fork)
...
- **Без власного токена** — нуль емісії, нуль волатильності, нуль регуляторного тягаря.
...
Сетлмент, анкеринг даних, ставки й споживацький UX зазвичай живуть як окремі
горизонтальні сервіси. XSight збирає їх **вертикально** в один продукт — кожен шар
спроєктований під єдиний домен (футбол) і єдиний чейн (X Layer):

| Шар | Що робить XSight |
|---|---|
| Сетлмент | доменний (футбол) bonded-оракул: propose→challenge→finalize — не generic |
| Анкеринг даних | multi-source quorum + evidence-хеші on-chain |
| Агрегація фікстур | хешує й анкорить результати, не перепродає сирі дані |
| Ставки | parimutuel — без LP і «дому», власний оракул |
| UX ринку | WC-themed, X Layer-native, з вбудованим AI-пундитом |
...
| Блокчейн | X Layer RPC, OKB, USDC, ethers/viem, solc, Hardhat | мережа, контракти, тести | — |
